...
_DEFAULT_LOGS_DIR = Path(__file__).parent / "logs"
_DEFAULT_LOGS_DIR.mkdir(exist_ok=True)  # Ensure default logs dir exists
...
def get_logger(name: str, level: int = None) -> logging.Logger:
    ...
    # Use internal defaults for logging configuration
...
def write_summary(stage_name: str, stats: dict, out_path: str, format_template: str = "default") -> None:
    ...
    Args:
        stage_name: Name of the stage (e.g., "Stage 04: EMG, Forceplate, and COM Data Merging")
        stats: Dictionary containing statistics and metadata to include
        out_path: Output file path
        format_template: Template style to use
    """
    import datetime
    from pathlib import Path
    ...
    # Use standard template format based on Stage 04/05 patterns
    with open(out_path, 'w', encoding='utf-8-sig') as f:
        ...
        for key, value in stats.items():
            if key != 'timestamp':  # Skip timestamp as it's already written
...
def run_parallel(
    ...
    # Use ThreadPoolExecutor for parallel processing (Windows-compatible)
    results = []
