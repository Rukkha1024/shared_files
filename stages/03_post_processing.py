...
def _standardize_join_keys_lf(lf: pl.LazyFrame, keys: Sequence[str]) -> pl.LazyFrame:
    casts: list[pl.Expr] = []
...
if __name__ == "__main__":
    # If run directly without arguments, use the run() function
    # Otherwise use main() for command line interface
    import sys
    if len(sys.argv) == 1:
        # No arguments provided, use run() function
        success = run()
        exit(0 if success else 1)
    else:
        # Arguments provided, use main() function
        exit(main())
